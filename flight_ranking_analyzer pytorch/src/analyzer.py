...
class FlightRankingAnalyzer:
    ...
    def __init__(self, 
        ...
        if use_gpu and torch.cuda.is_available():
            self._log(f"使用GPU: {torch.cuda.get_device_name(0)}")
        else:
            self._log("使用CPU")
        
        self._log("FlightRankingAnalyzer (PyTorch版本) 初始化完成")
    ...
    def _extract_feature_importance(self, model: BaseRanker, model_name: str, 
        ...
        try:
            if hasattr(model, 'feature_importances_'):
                ...
                # 对于PyTorch神经网络模型，使用梯度作为特征重要性
                importance = self._compute_neural_importance_pytorch(model, X_test)
                self.feature_importance_results[model_name] = {
                    'importance': importance,
                    'feature_names': feature_names
                }
    ...
    def _compute_neural_importance_pytorch(self, model: BaseRanker, X: np.ndarray, 
        ...
        try:
            ...
            self._log(f"计算PyTorch神经网络特征重要性时出错: {str(e)}")
            # 返回均匀分布作为后备方案
            return np.ones(X.shape[1]) / X.shape[1]
    ...
    def _compute_shap_values(self, model: BaseRanker, model_name: str, 
        ...
        try:
            ...
            if model_name in ['XGBRanker', 'LGBMRanker', 'LambdaMART', 'ListNet']:
                ...
                # 对于PyTorch神经网络模型，使用GradientExplainer
                try:
                    # 创建包装函数，让SHAP能够使用PyTorch模型
                    ...
                    explainer = shap.KernelExplainer(pytorch_predict, X_sample[:100])
                    shap_values = explainer.shap_values(X_sample)
                except Exception as kernel_e:
                    self._log(f"KernelExplainer失败，跳过SHAP计算: {str(kernel_e)}")
                    return
            else:
