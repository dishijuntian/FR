...
class AutoTuner:
    ...
    def __init__(self, 
        ...
        # PyTorch特定设置
        self.is_pytorch_model = model_name in ['NeuralRanker', 'RankNet', 'TransformerRanker']
        
        self._log(f"AutoTuner (PyTorch版本) 初始化完成: {model_name}")
        if self.is_pytorch_model:
            self._log(f"检测到PyTorch模型，使用设备: {DEVICE}")
    ...
    def _objective(self, trial: optuna.Trial) -> float:
        ...
        try:
            ...
            # 训练模型
            if self.is_pytorch_model:
                # 对于PyTorch模型，使用较少的epochs以加速调优
                train_params = params.copy()
                train_params['epochs'] = min(train_params.get('epochs', 10), 5)  # 限制epochs
                train_params['verbose'] = 0  # 减少输出
                
                model.fit(self.X_train, self.y_train, self.train_groups, **train_params)
            else:
...
class PyTorchModelPruner:
    ...
    def should_prune(self, trial: optuna.Trial, score: float) -> bool:
        """
        判断是否应该剪枝当前试验
...
def create_auto_tuner(model_name: str, 
    ...
    if model_name in ['NeuralRanker', 'RankNet', 'TransformerRanker']:
        # 减少PyTorch模型的试验次数以节省时间
        n_trials = min(n_trials, 30)
        if logger:
            logger.info(f"PyTorch模型 {model_name} 调优试验次数调整为: {n_trials}")
...
def optimize_pytorch_model_efficiently(model_name: str,
                                     X_train: np.ndarray,
                                     y_train: np.ndarray,
                                     train_groups: List[int],
                                     X_val: np.ndarray,
    ...
    Returns:
        Dict: 最优参数
    """
    if model_name not in ['NeuralRanker', 'RankNet', 'TransformerRanker']:
        raise ValueError(f"{model_name} 不是PyTorch模型")
    
    # 创建专门针对PyTorch模型的调优器
    search_space = Config.TUNING_SEARCH_SPACES.get(model_name, {}).copy()
    
    # 为快速调优调整搜索空间
    if 'epochs' in search_space:
        # 减少epochs的搜索范围
        if isinstance(search_space['epochs'], dict):
            search_space['epochs']['high'] = min(search_space['epochs']['high'], 10)
        elif isinstance(search_space['epochs'], list):
            search_space['epochs'] = [e for e in search_space['epochs'] if e <= 10]
    
    tuner = AutoTuner(
        model_name=model_name,
        search_space=search_space,
        objective_metric='hit_rate',
        n_trials=n_trials,
        timeout=3600,  # 1小时超时
        logger=logger
    )
